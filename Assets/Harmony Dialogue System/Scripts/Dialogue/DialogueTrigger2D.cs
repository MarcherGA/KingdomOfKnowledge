...
namespace HarmonyDialogueSystem
{
    ...
    public class DialogueTrigger2D : MonoBehaviour
    {
        ...
        [Tooltip("Button which will trigger the dialogue")]
        ...
        private void Awake()
        {
            ...
            DialogueManager.instance.dialogueSystemOnEvent.AddListener(TurnOnExitEvents);
        ...
        void Start()
        {
            triggerButton?.onClick.AddListener(EnterDialogueTrigger);
            InputManagerDialogue.instance.onInteract += EnterDialogueTrigger;
        }


        public void EnterDialogueTrigger()
        {
            if (playerInRange)
            {
